...
NewSoftSerial newSrial(BT_RX, BT_TX);
...
int SBSBT001::read(byte *buff, int len, unsigned int nakLimit)
{
  if (!connected) {
    response = "";
    msg_length = 0;
...
int SBSBT001::write(byte *buff, int len)
{
  ...
  for (int i = 0; i < len; i++) {
//      Serial.println(String(buff[i], DEC));
